...
func (w *Worker) Execute() error {
	...
	for _, model := range accessTemplate.Mappings {
		...
		modelValue, err := dm.ExecExpressionWithPrecision(model.Expression, args, model.Type, model.Precision)
		if err != nil {
			return err
		}
	...
	if err := w.ctx.ReportDevicePropertiesWithFilter(w.device.info, r); err != nil {
		return err
	}
	return nil
}
