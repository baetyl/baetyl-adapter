...
func (w *Worker) Execute() error {
	...
	for _, model := range accessTemplate.Mappings {
		...
		modelValue, err := dmcontext.ExecExpressionWithPrecision(model.Expression, args, model.Type, model.Precision)
		if err != nil {
			return err
		}
	...
	if err := w.ctx.ReportDevicePropertiesWithFilter(w.slave.dev, r); err != nil {
